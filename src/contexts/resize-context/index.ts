...
export const ResizeContext = createContext<ResizeContextType | null>({
    transformEducationHandlers: {
        goPrev: () => {},
        goNext: () => {},
        goToIdx: (_: number) => console.log(_),
    },
    transformEducationStyle: {
        sliderTransform: '',
        paginationLeftBtn: '',
        paginationRightBtn: '',
        dotActiveList: [] as boolean[],
    },
    iconSize: 30,
})
...
export const useResizeContext = () => {
    ...
    if (!ctx) throw new Error('ResizeContextProvider 안에서 사용해주세요')
    return ctx
}
